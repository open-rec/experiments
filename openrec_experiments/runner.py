...
from .datasets.ebnerd.runner_features import (
    history_inputs, fixed_history_features, apply_semantic_title_fallback,
    contextual_features, interaction_features, relative_temporal_features,
    long_term_user_features, topic_entity_affinity_features,
    candidate_statistical_ranks, session_exposure_features,
    past_candidate_exposure_features, past_candidate_engagement_features,
)
...
def run(config, output):
    ...
    run_manifest = {
        ...
        "source_files": {str(p.relative_to(experiment_root)): digest(p) for p in
                         (experiment_root / "openrec_experiments").rglob("*.py")},
    ...
    try:
        ...
        if model_type == "popularity":
            ...
            if model_type == "lightgbm":
                ...
                lightgbm_class = (
                    LightGBMBinaryModel
                    if get_dataset(config["dataset"]).ranking_objective == "binary"
                ...
                run_manifest["ranking_objective"] = (
                    get_dataset(config["dataset"]).ranking_objective
                )
            else:
